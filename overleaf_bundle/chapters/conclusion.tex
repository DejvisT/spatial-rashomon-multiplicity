...
This makes predictive multiplicity a practical reliability issue rather than only an abstract property of model classes. Predictions in hotspot regions should therefore be interpreted as more sensitive to model choice than predictions in stable regions, and may warrant additional caution, monitoring, or review in high-stakes settings. At the same time, the goal is not necessarily to eliminate all multiplicity: the Rashomon set is informative precisely because it reveals where equally defensible modeling choices lead to different outcomes.
...
\begin{itemize}
    ...
    \item \textbf{Multiplicity-aware deployment policies.} Hotspot information could be incorporated into selective prediction, abstention, or human-in-the-loop review workflows, so that cases that are especially sensitive to model choice are handled differently from stable ones.
