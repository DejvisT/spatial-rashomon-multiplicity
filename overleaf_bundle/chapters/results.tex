...
Figure~\ref{fig:null_summary} provides a cross-seed summary of the null experiment. The left panel shows both variance-based and conflict-based Moran's $I$: observed values (boxplots) versus null mean $\pm$ std for each dataset. The right panel displays z-scores, $z = (\text{observed} - \mu_{\text{null}}) / \sigma_{\text{null}}$, for both metrics. All COMPAS and German Credit z-scores exceed the $z = 1.96$ threshold, confirming robust significance across seeds. Breast Cancer z-scores are lower but mostly above the threshold, consistent with weaker but still detectable spatial structure.
