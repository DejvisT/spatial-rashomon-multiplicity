...
These datasets differ in size, domain, and feature composition. They therefore provide a
basic check of whether the proposed spatial multiplicity analysis behaves similarly across
different tabular classification settings.
...
The primary observation-wise multiplicity metric is predictive variance,
\[
...
In the PySAL implementation, the input variable is standardized before local Moran
statistics are computed. Let
\[
...
For each observation, a permutation test compares the observed local statistic with a
reference distribution obtained by randomly rearranging neighboring values. This produces
one local $p$-value per observation. Because many local tests are performed at once, the
Benjamini--Hochberg procedure is applied to control the false discovery rate
\citep{benjamini_hochberg_1995}. Observations that remain significant after this
...
Global Moran's $I$ significance is assessed with 999 permutations. Local Moran (LISA)
also uses 999 permutations per observation, and the Benjamini--Hochberg correction is
applied at $\alpha=0.05$ when labeling HH and LL clusters.

\subsection{Permutation Null Model}
\label{sec:meth-null}
\label{sec:exp-null}

The prediction-permutation null tests whether the observed spatial organization of
predictive variance is stronger than expected when predictions are no longer aligned with
feature-space location. The null hypothesis is that the model-wise marginal prediction
distributions are preserved, but their alignment with the feature-space graph is random.

The null operates on the prediction matrix $P \in \mathbb{R}^{K \times N}$. For each
model $m$ independently, the vector $(\hat{p}_{m1},\dots,\hat{p}_{mN})$ is randomly
permuted across observations. This within-model permutation preserves each model's
...
variance values is therefore attributable to chance under this null model.
...
The default global Rashomon approximation uses $K=25$. Sensitivity to this choice is
examined by varying
$K \in \{5, 10, 15, 20, 25, 30, 35, 40, 45, 50\}$ using global top-$K$ selection. For each $K$,
...
To test whether spatial multiplicity is driven by probability miscalibration, Platt scaling
and isotonic regression are applied to the predictions of each model in the global
Rashomon set. For each model, a logistic regression is fitted on validation-set predicted
probabilities and validation labels, and the resulting mapping is applied to that model's
test-set predictions. The predictive-variance and spatial pipeline is then rerun on both
uncalibrated and calibrated test predictions. We record changes in mean variance,
Moran's $I$, HH/LL masks, connected components, and the Jaccard similarity between
hotspot masks before and after calibration. Synthetic datasets are excluded from this
analysis because they follow a separate data-generating design.
...
known ambiguous regions. The single-island design contains two well-separated stable
Gaussian blobs and one ambiguous island, with points drawn uniformly in a disk and weak
XOR-like conditional probability $P(y=1\mid x)=0.5\pm\delta$. The three-islands design
contains two stable blobs, three ambiguous islands, and a small number of isolated
outliers with known region labels. The same training pipeline is applied: a 60/20/20
split, the same five model families, and top-$K$ selection by validation Brier score.
Recovery of the ground-truth ambiguous regions by LISA HH classification is evaluated
using precision, recall, and Jaccard overlap. The prediction-permutation null is used to
verify that observed Moran's $I$ is significant.

\subsection{Attribution and Interpretation Analyses}
\label{sec:framework-attribution}
...
\]
A value close to one means that most predictive variance is explained by differences
between model families. A lower value means that a larger share remains within families,
...
After separating the between-family component, the analysis asks which hyperparameters
explain disagreement within a given model family. For family $g$, let $\bar{p}^{(g)}_i$ be
...
To assess whether hyperparameter effects differ inside multiplicity hotspots, the same
...
disagreement inside HH regions, while negative values indicate lower importance inside
...
Attribute disagreement to modeling choices & Global vs per-family sets, family importance, within-family $V_m$ decomposition & Sections~\ref{sec:results-global-vs-family} and~\ref{sec:results-hp} \\
