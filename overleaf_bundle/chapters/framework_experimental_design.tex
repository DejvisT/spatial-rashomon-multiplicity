...
Let \((x_i,y_i)_{i=1}^N\) denote a supervised binary classification dataset, where
\(x_i\) is the feature vector of observation \(i\) and \(y_i \in \{0,1\}\) is the binary
target. After preprocessing, the feature matrix is represented in a numeric space. Let $\mathcal{F}$ denote a finite pool of
...
probabilities $\hat{p}_f(x_i) \in [0,1]$ for observations $i=1,\dots,N$.
...
All models are implemented  using \texttt{scikit-learn}
\citep{pedregosa2011scikit} in Python. The candidate pool contains five model families: logistic
...
In all experiments, the candidate pool is larger than \(K\), so \(|\mathcal{R}_K|=K\).
...
\[
P \in \mathbb{R}^{K \times N_{\text{test}}},
\]
where \(K\) is the number of retained models and \(N_{\text{test}}\) is the number of test observations.
After selecting the retained models \(\mathcal{R}_K = \{f_{(1)}, \ldots, f_{(K)}\}\), we index them by
\(m = 1,\ldots,K\) and write
\[
\hat{p}_{mi} := \hat{p}_{f_{(m)}}(x_i)
\]
for the predicted probability assigned to test observation \(i\) by retained model \(m\).

The primary observation-wise multiplicity metric is predictive variance. For each test
observation \(i\), let
\[
\bar{p}_i =
\frac{1}{K}
...
\hat{p}_{mi}
\]
denote the mean predicted probability across the retained top-\(K\) models. The
observation-wise predictive variance is computed as the population variance over the
...
\quad i=1,\dots,N_{\mathrm{test}}.
\]
Thus, the denominator is \(K\) rather than \(K-1\). This convention matches the empirical
...
summaries. Mean variance averages $v_i$ across test observations. We report a probability-space
analogue of ambiguity, defined as the test-set average of
the pointwise prediction range across Rashomon models,
\[
\frac{1}{N_{\mathrm{test}}}
\sum_{i=1}^{N_{\mathrm{test}}}
\left(
\max_{m=1,\dots,K} \hat{p}_{mi}
-
\min_{m=1,\dots,K} \hat{p}_{mi}
\right).
\] The disagreement rate is the fraction of
...
Let
\[
A \in \{0, 1\}^{N_{\mathrm{test}}\times N_{\mathrm{test}}}
\]
...
\[
W_{ij} = \frac{A_{ij}}{\sum_{j'=1}^{N_{\mathrm{test}}} A_{ij'}},
\quad i=1,\dots,N_{\mathrm{test}}.
\]
After this normalization, each row of $W$ sums to one. Therefore, for an
observation-level quantity $\mathbf{v}=(v_1,\dots,v_{N_{\mathrm{test}}})^\top$, the spatial lag
\[
[W\mathbf{v}]_i = \sum_{j=1}^{N_{\mathrm{test}}} W_{ij}v_j
\]
...
Let $\mathbf{v}=(v_1,\dots,v_{N_{\mathrm{test}}})^\top$ denote the predictive variance values on the test set,
and let $\bar{v}$ be their mean. Because $W$ is row-standardized, Moran's $I$ can be
written as
\[
I =
\frac{\sum_{i=1}^{N_{\mathrm{test}}} \sum_{j=1}^{N_{\mathrm{test}}} W_{ij}(v_i-\bar{v})(v_j-\bar{v})}
{\sum_{i=1}^{N_{\mathrm{test}}} (v_i-\bar{v})^2}.
\]
...
local clusters. Local Moran's $I$ is the specific LISA statistic used in this thesis; LISA 
denotes the broader class of local spatial association measures.
...
\[
[W\mathbf{z}]_i = \sum_{j=1}^{N_{\mathrm{test}}} W_{ij}z_j.
\]
...
\citep{benjamini_hochberg_1995} at a default false-discovery-rate level of
$\alpha = 0.05$ before assigning final LISA labels. Sensitivity to this threshold is
examined for the synthetic validation experiments in Appendix~\ref{app:synthetic-fdr}.
...
region in feature space. Unless stated otherwise, components smaller than five HH observations
are discarded to avoid treating isolated fragments as substantive hotspot regions.
...
The null operates on the prediction matrix $P \in \mathbb{R}^{K \times N_{\mathrm{test}}}$. For each model
$m$ independently, the vector $(\hat{p}_{m1},\dots,\hat{p}_{mN_{\mathrm{test}}})$ is randomly permuted
...
prediction matrix, and global Moran's $I$ is recalculated. Any spatial clustering in the recomputed
variance field is therefore interpreted relative to
...
For each of \(R\) permutations, observation-wise predictive variance and global
Moran's \(I\) are recomputed. The empirical one-sided \(p\)-value is
\[
p =
\frac{
    1 + \sum_{r=1}^{R}
    \mathds{1}\!\left\{ I_{\mathrm{null}}^{(r)} \ge I_{\mathrm{obs}} \right\}
}{
    R + 1
}.
\]
...
this null deliberately removes the feature-space alignment of the predictions, the recomputed
variance field should no longer preserve systematic feature-space alignment, apart from finite-sample variation. This also
...
The default global Rashomon approximation uses $K=25$. Sensitivity to this choice is examined
by varying the Rashomon set size over
\[
...
model in the global Rashomon set. For Platt scaling, a logistic regression is fitted on 
validation-set predicted probabilities and validation labels; for isotonic calibration, 
an isotonic regression mapping is fitted on the same validation predictions and labels. 
The resulting mapping is then applied to that model's test-set predictions.
...
$P(y = 1 \mid x) \approx 0.5$. This design validates the pipeline in the classical
...
denoted \(\mathcal{R}_K\), combining all model families. The per-family construction
selects the top-\(K=25\) models separately within each family, denoted
\(\mathcal{R}^{(g)}_K\) for family \(g\). Comparing these constructions shows whether
pooling across families hides family-specific multiplicity patterns.

\paragraph{Family importance.}
Predictive variance is decomposed into a between-family component and a within-family
component. Let \(\mathcal{G}\) denote the set of model families. For each family
\(g \in \mathcal{G}\), let
\[
\mathcal{R}^{(g)}_K =
\{f_{(1)}^{(g)}, \ldots, f_{(K)}^{(g)}\}
\]
denote the top-\(K\) models within that family. For observation \(i\), we write
\[
\hat{p}_{mi}^{(g)} := \hat{p}_{f_{(m)}^{(g)}}(x_i),
\qquad m = 1,\ldots,K,
\]
for the predicted probability assigned to observation \(i\) by the \(m\)-th retained model
of family \(g\). The mean prediction of family \(g\) is
\[
\bar{p}^{(g)}_i =
\frac{1}{K}
\sum_{m=1}^{K}
...
For a set of observations \(\mathcal{S}\), such as all test observations, HH observations,
or non-HH observations, the between-family component is
\[
\mathrm{Var}_{\mathrm{between}}(\mathcal{S})
=
\frac{1}{|\mathcal{S}|}
\sum_{i\in \mathcal{S}}
\frac{1}{|\mathcal{G}|}
\sum_{g\in \mathcal{G}}
\left(\bar{p}^{(g)}_{i}-\bar{p}_{i}\right)^2 .
\]
The total predictive variance across all family-specific model sets is
\[
\mathrm{Var}_{\mathrm{total}}(\mathcal{S})
=
\frac{1}{|\mathcal{S}|}
\sum_{i\in \mathcal{S}}
\frac{1}{|\mathcal{G}|K}
\sum_{g\in \mathcal{G}}
\sum_{m=1}^{K}
\left(\hat{p}^{(g)}_{mi}-\bar{p}_{i}\right)^2 .
\]
The corresponding within-family component is
\[
\mathrm{Var}_{\mathrm{within}}(\mathcal{S})
=
\mathrm{Var}_{\mathrm{total}}(\mathcal{S})
-
\mathrm{Var}_{\mathrm{between}}(\mathcal{S}).
...
\[
\mathrm{FI}(\mathcal{S}) =
\frac{\mathrm{Var}_{\mathrm{between}}(\mathcal{S})}
{\mathrm{Var}_{\mathrm{total}}(\mathcal{S})}.
\]
A value close to one means that most observed predictive variance is attributable to
differences between model-family mean predictions. A lower value means that a larger
share remains within families, for example because of hyperparameter variation. Because
each family contributes the same number of models, this decomposition gives equal weight
to each model family.
...
After separating the between-family component, the analysis asks which grouped
hyperparameter values are associated with disagreement within a given model family.
Using the same family mean prediction \(\bar{p}^{(g)}_i\) as above, the model-level
disagreement contribution of retained model \(m=1,\ldots,K\) is
\[
V^{(g)}_m =
...
Let \(\mathcal{B}^{(g)}_h\) denote the set of groups induced by hyperparameter
\(h\) within family \(g\). For example, these groups may correspond to exact
categorical values or to quantile bins for numeric hyperparameters. Let
\(M_b \subseteq \{1,\ldots,K\}\) denote the retained model indices belonging to
group \(b \in \mathcal{B}^{(g)}_h\), and let \(n_b = |M_b|\). The mean
model-level disagreement within group \(b\) is
\[
\bar V_b^{(g)}
=
\frac{1}{n_b}
...
V_m^{(g)},
\]
and the overall mean model-level disagreement within family \(g\) is
\[
\bar V^{(g)}
=
\frac{1}{K}
...
V_m^{(g)}.
\]
The total variation in model-level disagreement is
\[
...
\right)^2.
\]
The between-group component can first be written as a sum over retained models,
where \(b(m)\) denotes the group of hyperparameter \(h\) to which model \(m\)
belongs:
\[
\frac{1}{K}
\sum_{m=1}^{K}
\left(
\bar V_{b(m)}^{(g)}-\bar V^{(g)}
\right)^2.
\]
Since all models in the same group \(b\) share the same group mean
\(\bar V_b^{(g)}\), the identical term
\((\bar V_b^{(g)}-\bar V^{(g)})^2\) appears once for each of the \(n_b\)
models in that group. Therefore, the sum over models can be rewritten as a
...
=
\frac{1}{K}
\sum_{b\in \mathcal{B}^{(g)}_h}
n_b
\left(
\bar V_b^{(g)}-\bar V^{(g)}
\right)^2.
\]
The right-hand side is the between-group component,
\[
\mathrm{Var}^{(g)}_{\mathrm{between}}(h)
=
\frac{1}{K}
\sum_{b\in \mathcal{B}^{(g)}_h}
n_b
...
\right)^2.
\]
Hyperparameter importance is then
\[
\rho^{(g)}(h)
=
...
{\mathrm{Var}^{(g)}_{\mathrm{total}}}.
\]
If \(\mathrm{Var}^{(g)}_{\mathrm{total}}=0\), the corresponding hyperparameter
importance is undefined and excluded from aggregation. Results are aggregated over
families and random seeds by mean importance, standard deviation, and the number of
valid seeds.

\paragraph{Hotspot-specific hyperparameter shifts.}
To assess whether these descriptive hyperparameter associations differ inside multiplicity
hotspots, the same \(V_m\)-based importance calculation is repeated on HH observations.
If \(\mathcal{H}\) denotes the HH set for the current run, defined from the global
top-\(K\) variance field, the hotspot-specific model-level disagreement contribution is
\[
V^{(g)}_{m,\mathrm{HH}}
=
\frac{1}{|\mathcal{H}|}
...
\left(\hat{p}^{(g)}_{mi}-\bar{p}^{(g)}_i\right)^2 .
\]
The quantity \(\rho^{(g)}_{\mathrm{HH}}(h)\) is computed from
\(V^{(g)}_{m,\mathrm{HH}}\) using the same group-wise decomposition as above, while
\(\rho^{(g)}_{\mathrm{all}}(h)\) uses all test observations. For each hyperparameter, the
...
\]
Positive values indicate that grouped values of hyperparameter \(h\) account for a larger
share of within-family disagreement inside HH regions for family \(g\). Because HH subsets
can be small, especially on German Credit, these estimates are interpreted descriptively
and reported with seed coverage where relevant. An auxiliary performance-control
meta-model diagnostic is reported in Appendix~\ref{app:meta-model-diagnostic}.
...
Rules are evaluated using support, purity, recall, and lift. Let \(T\) denote the test set,
let \(A \subseteq T\) denote the set of observations satisfying a rule, and let
\(H \subseteq T\) denote the relevant positive target set: the full HH set for global rules,
or one connected HH component for component-level rules. Support is defined as \(|A|\),
purity as
\[
\frac{|A \cap H|}{|A|},
\]
...
\frac{|A \cap H|/|A|}{|H|/|T|}.
\]
Thus, lift compares the rule purity with the base rate of the target set.

For global HH rules, \(H\) is the full HH set, so purity corresponds to
\(P(\mathrm{HH}\mid\mathrm{rule\ region})\). For component-level rules, \(H\) is the
corresponding connected component, so purity corresponds to
\(P(\mathrm{component}\mid\mathrm{rule\ region})\). The main rule analysis is reported for
COMPAS, where the hotspot structure is clearest and retained components are sufficiently
...
Figure~\ref{fig:analysis-pipeline} gives a visual overview of the empirical
analysis pipeline, from candidate model construction to hotspot detection,
robustness checks, and interpretation. Table~\ref{tab:framework-method-result-map}
then summarizes the main analysis goals, methods, and corresponding result sections.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{presentation_assets/analysis_pipeline_overview.png}
    \caption{Overview of the empirical analysis pipeline from the candidate model pool
    to spatial hotspot detection, robustness checks, and interpretation.}
...
\midrule
Measure where near-optimal models disagree
& Predictive variance across $\mathcal{R}_K$
& Section~\ref{sec:results-metrics} \\

Test whether disagreement is spatially clustered
& Feature-space $k$NN graph, Moran's $I$, prediction-permutation null
& Sections~\ref{sec:results-metrics}--\ref{sec:results-null} \\

Identify local high-multiplicity regions
& LISA HH labels and connected HH components
& Section~\ref{sec:results-spatial} \\

Assess stability and robustness
& Fixed-test-set stability, $K$ and $k$ sensitivity, calibration, alternative graphs
& Sections~\ref{sec:results-spatial} and~\ref{sec:results-robustness} \\

Summarize associations with modeling choices
& Global vs.\ per-family sets, family importance, within-family $V_m$ decomposition
& Section~\ref{sec:results-hp} \\

Validate and interpret hotspots
& Synthetic ground-truth designs and shallow-tree rule extraction
& Sections~\ref{sec:results-synthetic}--\ref{sec:results-rules} \\
\bottomrule
\end{tabular}
\end{table}
