...
\[
P \in \mathbb{R}^{K \times N_{\mathrm{test}}},
\]
where \(K\) is the number of retained models and \(N_{\mathrm{test}}\) is the number of
test observations. The entry \(\hat p_{mi}\) denotes the predicted probability assigned
to test observation \(i\) by retained model \(m\), for \(m=1,\ldots,K\).
...
$i$, let $\hat{p}^{(g)}_{mi}$ be the predicted probability from model
...
\[
\bar{p}_{i} = \frac{1}{|\mathcal{G}|}\sum_{g\in\mathcal{G}}\bar{p}^{(g)}_i.
\]
...
\sum_{m\in\mathcal{R}^{(g)}_K}(\hat{p}^{(g)}_{mi}-\bar{p}_{i})^2.
\]
...
share of within-family disagreement inside HH regions for family $g$. Because HH subsets can be small, especially on German Credit, these estimates
...
Rules are evaluated using support, purity, recall, and lift. Let $A$ denote the
set of observations satisfying a rule, and let $H$ denote the relevant positive
target set: the full HH set for global rules, or one connected HH component for
component-level rules. Support is defined as $|A|$, purity as
\[
\frac{|A \cap H|}{|A|},
\]
recall as
...
and lift as
\[
\frac{|A \cap H|/|A|}{|H|/N_{\mathrm{test}}}.
\]
