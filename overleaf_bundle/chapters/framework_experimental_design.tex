...
summaries. Mean variance averages $v_i$ across test observations. We report a probability-space analogue of ambiguity, defined as the test-set average of
...
region in feature space. Unless stated otherwise, components smaller than five HH observations are discarded to avoid treating isolated fragments as substantive hotspot regions.
...
this null deliberately removes the feature-space alignment of the predictions, the recomputed variance field should no longer preserve systematic feature-space alignment, apart from finite-sample variation. This also
...
model in the global Rashomon set. For Platt scaling, a logistic regression is fitted on validation-set predicted probabilities and validation labels; for isotonic calibration, an isotonic regression mapping is fitted on the same validation predictions and labels. The resulting mapping is then applied to that model's test-set predictions.
...
Table~\ref{tab:framework-method-result-map} summarizes the main analysis goals,
methods, and corresponding result sections.
