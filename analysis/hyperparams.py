...
def compute_within_family_hp_importance_on_subset(
    meta: pd.DataFrame,
    preds: np.ndarray,
    obs_mask: np.ndarray,
    hp_cols: Optional[Sequence[str]] = None,
    ...
    **kwargs: Any,
    ...
    """
    Convenience wrapper: within-family HP importance restricted to a subset of observations.

    Example: obs_mask = HH_mask from LISA to compute hotspot-specific drivers.
    """
    return compute_within_family_hp_importance(
        meta,
        preds,
        hp_cols=hp_cols,
...
# ---------------------------------------------------------------------------
# Backwards-compatible wrappers (used by your current notebook)
# ---------------------------------------------------------------------------

def variance_decomposition_by_hyperparameter(
    P: np.ndarray,
    meta: pd.DataFrame,
    hp_name: str,
    ...
    *,
    ...
    """
    Backwards-compatible API: decompose variance by one hyperparameter.

    This is now implemented as a thin wrapper around `variance_decomposition_by_groups`
    with stable grouping keys.
    """
    meta = ensure_hp_columns(meta)
    P = np.asarray(P)

    if model_family is not None:
        mask = (meta["model_name"] == model_family).values
        meta = meta.loc[mask].reset_index(drop=True)
        P = P[mask]

    hp_col = _resolve_hp_column(meta, hp_name)
    if hp_col is None:
        raise ValueError(
            f"Hyperparameter '{hp_name}' not found in meta. "
            f"Available hp_ columns: {[c for c in meta.columns if c.startswith('hp_')][:50]}"
        )

    P_use = _subset_observations(P, obs_mask)

    keys = np.array([make_hp_key(v) for v in meta[hp_col].values], dtype=object)
    if dropna:
        valid = keys != "nan"
        P_use = P_use[valid]
        keys = keys[valid]

    out = variance_decomposition_by_groups(P_use, keys, min_group_size=min_group_size)

    return {
        "var_total": out["var_total"],
        "var_between": out["var_between"],
        "var_within": out["var_within"],
        "ratio": out["ratio"],
        "hp_values": np.array(list(out["group_counts"].keys()), dtype=object),
        "hp_counts": out["group_counts"],
        "n_models_used": out["n_models_used"],
        "n_groups": out["n_groups"],
        "hp_col": hp_col,
    }


def hyperparameter_profiling(
    meta: pd.DataFrame,
    hp_name: str,
    epsilon: float,
    model_family: Optional[str] = None,
    *,
    ...
    """
    Profile a hyperparameter by computing "local Rashomon sets" per value.

    For each hyperparameter value h:
      1) find best achievable validation loss L*(h) among models with HP=h
      2) local Rashomon set = models with HP=h and loss <= L*(h) + epsilon
      3) report its size and fraction

    Notes
    -----
    - This uses the *candidate pool in `meta`*, not only the global Rashomon set.
      If you call it on a meta that is already filtered to the global Rashomon set,
      then local_rashomon_fraction may be trivially 1.0.
    """
    meta = ensure_hp_columns(meta)

    if model_family is not None:
        meta = meta.loc[meta["model_name"] == model_family].copy()

    hp_col = _resolve_hp_column(meta, hp_name)
    if hp_col is None:
        raise ValueError(f"Hyperparameter '{hp_name}' not found in meta")

    loss_col = _choose_loss_col(meta, loss_col=loss_col)

    keys = meta[hp_col].apply(make_hp_key)
    if dropna:
        meta = meta.loc[keys != "nan"].copy()
        keys = meta[hp_col].apply(make_hp_key)

    meta = meta.copy()
    meta["_hp_key"] = keys.values

    unique_keys = pd.unique(meta["_hp_key"])

    results = []
    for key in unique_keys:
        sub = meta.loc[meta["_hp_key"] == key]
        if sub.empty:
            continue
        best_loss = float(sub[loss_col].min())
        local = sub.loc[sub[loss_col] <= best_loss + epsilon]
        results.append({
            "hp_value": key,
            "best_loss": best_loss,
            "local_rashomon_size": int(len(local)),
            "total_models": int(len(sub)),
            "local_rashomon_fraction": float(len(local) / len(sub)) if len(sub) else 0.0,
        })

    return pd.DataFrame(results).sort_values("best_loss").reset_index(drop=True)
...
def compute_hp_importance(
    ...
    *,
    ...
    """
    Compute hyperparameter "importance" via variance decomposition.

    This function is kept for backwards compatibility with the notebook, but:
    - If you run it across multiple families, it can still be confounded.
    - Prefer Option A:
        compute_family_importance(...)
        compute_within_family_hp_importance(...)

    By default (dropna=True), this restricts each HP analysis to models where the HP
    is actually defined (i.e., excludes 'nan').

    Returns columns compatible with your existing notebook plus robust extras:
      - mean_var_between_ratio (mean of per-observation ratios)
      - ratio_of_sums (sum(var_between)/sum(var_total))
      - median_ratio, p90_ratio
      - n_models_used, n_groups
    """
    meta = ensure_hp_columns(meta)
    P = np.asarray(P)
    ...
    for hp_col in hp_cols:
        ...
        unique_keys = pd.unique(keys_use)
        if len(unique_keys) < min_groups:
            continue

        out = variance_decomposition_by_groups(P_use, keys_use, min_group_size=min_group_size)
        if out["n_models_used"] < min_models or out["n_groups"] < min_groups:
            continue

        summ = _summarize_decomposition(out)

        # Backwards-compatible "within ratio" (mean var_within/var_total on stable denom)
        denom = np.maximum(out["var_total"], EPS_RATIO)
        within_ratio_vec = out["var_within"] / denom
        mean_within_ratio = float(np.nanmean(within_ratio_vec))

        results.append({
            "hyperparameter": hp_name,
            "mean_var_between_ratio": summ["mean_ratio"],
            "ratio_of_sums": summ["ratio_of_sums"],
            "median_ratio": summ["median_ratio"],
            "p90_ratio": summ["p90_ratio"],
            "mean_var_within_ratio": mean_within_ratio,
            # Compatibility + additional bookkeeping
            "n_values": int(out["n_groups"]),
            "n_groups": int(out["n_groups"]),
            "n_models_total": int(P.shape[0]),
            "n_models_used": int(out["n_models_used"]),
        })
    ...
    df = df.sort_values("ratio_of_sums", ascending=False).reset_index(drop=True)
    return df
