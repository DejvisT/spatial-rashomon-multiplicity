...
def stability_importance_tables(
    grp: pd.DataFrame,
    hp_cols: Sequence[str],
    target_col: str,
    seed_col: str,
    ...
    per_rep: List[pd.DataFrame] = []
    for rep in range(n_reps):
        sub = _bootstrap_resample_group(grp, seed_col, rng)
        X, y, hp_use, _ = prepare_xy(sub, hp_cols, target_col, seed_col if seed_col in sub.columns else None)
        if X is None or y is None:
            continue
        pipe = make_rf_pipeline(X, random_state=random_state + rep, n_estimators=80)
        try:
            pipe.fit(X, y)
        except Exception:
            continue
        gdf = grouped_importances_from_pipe(pipe, X, "validation_brier", hp_use)
        gdf["rep"] = rep
        per_rep.append(gdf)
    ...
    for gdf in per_rep:
        rep_id = int(gdf["rep"].iloc[0])
        d = gdf.drop(columns=["rep"]).sort_values("importance", ascending=False).reset_index(drop=True)
        for i, row in enumerate(d.itertuples(index=False), start=1):
            rank_rows.append({"rep": rep_id, "feature_group": row.feature_group, "rank": i})

    rank_df = pd.DataFrame(rank_rows)
    all_imp = pd.concat(per_rep, ignore_index=True)
    rep_ids = sorted(rank_df["rep"].unique().tolist())
    n_succ = len(rep_ids)
    rows = []
    for fg, chunk in all_imp.groupby("feature_group"):
        rsub = rank_df[rank_df["feature_group"] == fg]
        ranks = rsub["rank"].astype(float).tolist()
        c_top1 = 0
        c_top3 = 0
        for rep_id in rep_ids:
            top1_row = rank_df[(rank_df["rep"] == rep_id) & (rank_df["rank"] == 1)]
            if not top1_row.empty and str(top1_row.iloc[0]["feature_group"]) == str(fg):
                c_top1 += 1
            top3f = set(rank_df[(rank_df["rep"] == rep_id) & (rank_df["rank"] <= 3)]["feature_group"].astype(str))
            if str(fg) in top3f:
                c_top3 += 1
        freq_top1 = float(c_top1 / max(n_succ, 1))
        freq_top3 = float(c_top3 / max(n_succ, 1))
        rows.append(
            {
                "feature_group": fg,
