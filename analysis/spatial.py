...
def getis_ord_gi_star(
    ...
    Unlike LISA which mixes clusters and outliers, Gi* focuses specifically
    on concentrations of high (or low) values — a cleaner hotspot definition.
    
    Parameters
    ----------
    v : array of shape (n,) — the variable to analyze (e.g. pointwise variance)
    W : sparse weight matrix (n, n) — should include self-neighbors for Gi*
    permutations : number of permutation tests
    alpha : significance level
    fdr : apply Benjamini-Hochberg FDR correction
    seed : random seed
    
    Returns
    -------
    DataFrame with columns: Gi_star, p_value, significant, hotspot
    """
    rng = np.random.RandomState(seed)
    if not isinstance(W, sparse.csr_matrix):
        W = sparse.csr_matrix(W)
    ...
    # Add self-connections for Gi* (as opposed to Gi which excludes self)
    W_star = W + sparse.eye(n, format='csr')
    # Re-normalize rows
    row_sums = np.array(W_star.sum(axis=1)).flatten()
    row_sums[row_sums == 0] = 1.0
    ...
    if S < 1e-12:
        return pd.DataFrame({
            ...
            "hotspot": np.zeros(n, dtype=bool),
        })
    
    # Gi* = (sum_j w_ij * x_j - x_mean * sum_j w_ij) / (S * sqrt((n * sum_j w_ij^2 - (sum_j w_ij)^2) / (n - 1)))
    Gi_star = np.zeros(n)
    for i in range(n):
        w_i = np.array(W_star[i].todense()).flatten()
        wi_sum = w_i.sum()
        wi_sq_sum = (w_i ** 2).sum()
        
        numerator = w_i @ v - x_mean * wi_sum
        denom_inner = (n * wi_sq_sum - wi_sum ** 2) / (n - 1)
        if denom_inner <= 0:
            Gi_star[i] = 0.0
        else:
            denominator = S * np.sqrt(denom_inner)
            Gi_star[i] = numerator / denominator if denominator > 1e-12 else 0.0
    ...
    for i in range(n):
        ...
        for _ in range(permutations):
            ...
            num_perm = w_i @ v_perm - x_mean * wi_sum
            if denom_inner > 0:
                denom_perm = S * np.sqrt(denom_inner)
                gi_perm = num_perm / denom_perm if denom_perm > 1e-12 else 0.0
            else:
                gi_perm = 0.0
            if abs(gi_perm) >= abs(Gi_star[i]):
                perm_count += 1
        p_vals[i] = (perm_count + 1) / (permutations + 1)
...
# ---------------------------------------------------------------------
# Spatial correlogram (Moran's I at varying k)
# ---------------------------------------------------------------------

def spatial_correlogram(
    v: np.ndarray,
    X: np.ndarray,
    k_values: tuple = (5, 10, 15, 20, 30, 40, 50),
    permutations: int = 999,
    seed: int = 42,
) -> pd.DataFrame:
    """
    Compute Moran's I at different neighborhood sizes k to characterize
    the spatial scale of clustering (distance-decay analysis).
    
    Returns DataFrame with columns: k, moran_i, p_value
    """
    from sklearn.neighbors import NearestNeighbors
    ...
    for k in k_values:
        if k >= len(v):
            continue
        W = build_knn_graph(X, k=k, standardize=True)
        result = moran_global(v, W, permutations=permutations, seed=seed)
        rows.append({
            "k": k,
            "moran_i": result["I"],
