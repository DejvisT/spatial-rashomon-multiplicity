...
- `--copy-all-figures` — copy every PDF from `thesis_outputs/figures/` (and legacy `figures/`), not only those referenced in the thesis `.tex` files
- `--prune-presentation-figs` — remove PDFs under `presentation_assets/fig/` that are not referenced by the thesis or generated by the export script
