...
def copy_notebook_figures(
    ...
    *,
    ...
    :param prune_orphans: delete *.pdf in fig_dir not referenced by the thesis.
    """
    import shutil
    ...
    if copy_all:
        ...
        to_copy = [(n, by_name[n]) for n in sorted(thesis_refs) if n in by_name]
        missing_sources = thesis_refs - frozenset(by_name.keys())
        for n in sorted(missing_sources):
            print(
                f"Warning: thesis references {n} but no PDF found under "
                "thesis_outputs/figures/ (generate via notebooks)."
            )
    ...
    if prune_orphans:
        ...
        for f in sorted(fig_dir.glob("*.pdf")):
            if f.name not in thesis_refs:
                f.unlink()
                removed += 1
                print(f"  Pruned orphan {f.name}")
        if removed:
            print(f"Pruned {removed} PDF(s) not referenced by the thesis.")
        else:
