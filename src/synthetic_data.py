...
# ---------------------------------------------------------------------------
# Graduated ambiguity: two islands with different signal strengths + outliers
# ---------------------------------------------------------------------------


@dataclass
class SyntheticGraduatedGT:
    """Ground truth for the graduated-ambiguity synthetic dataset.

    region_id encoding:
        -1 = stable blob
         0 = strong-signal island  (high delta  -> large model disagreement)
         1 = moderate-signal island (lower delta -> smaller model disagreement)
        99 = scattered outlier      (p=0.5, but isolated -> high variance, no HH)
    """
    region_id: np.ndarray
    island_mask: np.ndarray
    strong_island_mask: np.ndarray
    moderate_island_mask: np.ndarray
    outlier_mask: np.ndarray
    stable_mask: np.ndarray
    p_true: np.ndarray
    island_centers: List[Tuple[float, float]]
    island_radii: List[float]
    island_deltas: List[float]


def make_synth_graduated_ambiguity(
    *,
    n_samples: int = 5000,
    stable_sep: float = 5.0,
    stable_std: float = 0.9,
    strong_center: Tuple[float, float] = (-6.0, 6.0),
    strong_radius: float = 2.5,
    strong_delta: float = 0.35,
    strong_frac: float = 0.15,
    moderate_center: Tuple[float, float] = (6.0, 6.0),
    moderate_radius: float = 2.5,
    moderate_delta: float = 0.20,
    moderate_frac: float = 0.15,
    p_outliers: float = 0.02,
    outlier_box: Tuple[float, float, float, float] = (-20, 20, -15, 15),
    outlier_min_dist: float = 2.0,
    outlier_p: float = 0.5,
    random_state: int = 0,
    shuffle: bool = True,
) -> Tuple[pd.DataFrame, pd.Series, Dict[str, list], SyntheticGraduatedGT]:
    """
    Synthetic dataset with *graduated* levels of ambiguity.

    Components:
      1. **Stable blobs** -- two well-separated Gaussians; models agree.
      2. **Strong island** -- XOR signal with high delta; tree-based models
         learn the pattern, linear models do not -> large predictive variance,
         strong HH cluster expected.
      3. **Moderate island** -- same XOR mechanism but lower delta -> smaller
         variance gap, weaker HH signal.
      4. **Scattered outliers** -- coin-flip labels, isolated in feature space
         -> high per-point variance but NO spatial clustering (should not form
         HH clusters; demonstrates the method distinguishes structured
         ambiguity from isolated noise).

    This design shows the spatial method correctly:
      - Finds strong HH where ambiguity is large and spatially concentrated.
      - Finds weaker/fewer HH where ambiguity is moderate.
      - Does NOT flag isolated outliers as HH despite their high variance.
    """
    if not (0.0 < strong_delta < 0.5):
        raise ValueError("strong_delta must be in (0, 0.5).")
    if not (0.0 < moderate_delta < 0.5):
        raise ValueError("moderate_delta must be in (0, 0.5).")

    rng = np.random.default_rng(random_state)

    n_strong = int(round(n_samples * strong_frac))
    n_moderate = int(round(n_samples * moderate_frac))
    n_out = int(round(n_samples * p_outliers))
    n_stable = n_samples - n_strong - n_moderate - n_out
    if n_stable <= 0:
        raise ValueError("Increase n_samples or reduce island/outlier fractions.")
    ...
    X_strong = _sample_uniform_disk(rng, n_strong, strong_radius, strong_center)
    p_strong = _weak_xor_probability(X_strong, strong_delta, strong_center)
    y_strong = rng.binomial(1, p_strong).astype(int)

    X_mod = _sample_uniform_disk(rng, n_moderate, moderate_radius, moderate_center)
    p_mod = _weak_xor_probability(X_mod, moderate_delta, moderate_center)
    y_mod = rng.binomial(1, p_mod).astype(int)

    existing = np.vstack([X_stable, X_strong, X_mod])
    X_out = _rejection_sample_outliers(
        rng, n_out, outlier_box,
        min_dist_to_any=outlier_min_dist, existing_points=existing,
    )
    p_out = np.full(n_out, float(outlier_p))
    y_out = rng.binomial(1, p_out).astype(int)

    X_all = np.vstack([X_stable, X_strong, X_mod, X_out])
    y_all = np.concatenate([y_stable, y_strong, y_mod, y_out])
    p_true_all = np.concatenate([p_true_stable, p_strong, p_mod, p_out])

    region_id = np.concatenate([
        np.full(n_stable, -1, dtype=int),
        np.full(n_strong, 0, dtype=int),
        np.full(n_moderate, 1, dtype=int),
        np.full(n_out, 99, dtype=int),
    ])

    island_mask = (region_id >= 0) & (region_id <= 1)
    strong_island_mask = region_id == 0
    moderate_island_mask = region_id == 1
    outlier_mask = region_id == 99
    stable_mask = region_id == -1
    ...
    if shuffle:
        ...
        y_all = y_all[perm]
        p_true_all = p_true_all[perm]
        region_id = region_id[perm]
        island_mask = island_mask[perm]
        strong_island_mask = strong_island_mask[perm]
        moderate_island_mask = moderate_island_mask[perm]
        outlier_mask = outlier_mask[perm]
        stable_mask = stable_mask[perm]
    ...
    gt = SyntheticGraduatedGT(
        region_id=region_id,
        island_mask=island_mask,
        strong_island_mask=strong_island_mask,
        moderate_island_mask=moderate_island_mask,
        outlier_mask=outlier_mask,
        stable_mask=stable_mask,
        p_true=p_true_all,
        island_centers=[strong_center, moderate_center],
        island_radii=[strong_radius, moderate_radius],
        island_deltas=[strong_delta, moderate_delta],
    )
    return X, y_series, feature_info, gt
